...
def fixed_cards(title, values, cls=""):
    cards = "".join(f'<article><b>{i + 1:02d}</b><p>{e(value)}</p></article>' for i, value in enumerate(values))
    return f'<section class="{e(cls)}"><div class="section-title"><span>FIXED VALUE</span><h2>{e(title)}</h2></div><div class="grid">{cards}</div></section>'


def client_html(data, fixed, ip):
...
@page{{size:A4;margin:0}}*{{box-sizing:border-box}}:root{{--ink:#10204b;--blue:#075ff2;--cyan:#08c8bb;--purple:#7566f2;--line:#d7e5f5}}body{{margin:0;background:#eaf2ff;color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif;line-height:1.55}}main{{width:210mm;min-height:297mm;margin:auto;background:white}}header,section{{padding:10mm 15mm;border-bottom:1px solid #e6eef8}}header{{position:relative;min-height:90mm;padding-top:18mm;color:white;background:linear-gradient(135deg,#082c79 0%,#0768eb 48%,#08bcae 100%)}}.header-copy{{position:relative;z-index:1;max-width:128mm}}.ip-hero{{position:absolute;right:12mm;bottom:10mm;width:44mm;height:44mm;display:block;object-fit:contain;object-position:center}}.eyebrow,.section-title span{{font-size:10px;font-weight:800;letter-spacing:1.3px;color:#09a999}}header .eyebrow{{color:#84fff2}}h1{{font-size:31px;line-height:1.2;margin:6px 0 12px}}.tags{{display:flex;gap:7px;flex-wrap:wrap;max-width:125mm}}.tags span{{font-size:11px;padding:4px 9px;border:1px solid rgba(255,255,255,.45);border-radius:5px;background:rgba(255,255,255,.12)}}.section-title{{display:flex;align-items:baseline;gap:10px;margin-bottom:12px}}h2{{font-size:20px;margin:0}}h3{{font-size:13px;margin:0 0 4px}}p,li{{font-size:12px}}.overview{{display:grid;grid-template-columns:1fr 1.25fr;gap:20px}}.risk-grid,.grid{{display:grid;grid-template-columns:repeat(2,1fr);gap:9px}}article{{padding:11px;border:1px solid var(--line);border-radius:6px;background:#fbfdff}}article b{{font-size:18px;color:var(--cyan)}}article p{{margin:3px 0;color:#5b6b82}}table{{width:100%;border-collapse:separate;border-spacing:0;border:1px solid var(--line);border-radius:6px;overflow:hidden;font-size:10px}}th,td{{padding:7px;border-right:1px solid var(--line);border-bottom:1px solid var(--line);vertical-align:top;text-align:left}}th:last-child,td:last-child{{border-right:0}}tr:last-child td{{border-bottom:0}}th{{background:#eaf4ff}}.assurance{{color:white;background:#10295f}}.assurance h2,.assurance .section-title span{{color:white}}.assurance .grid article{{background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.18)}}.assurance article p{{color:white}}.materials{{background:#edfffb}}@media(max-width:760px){{main{{width:100%}}.overview,.risk-grid,.grid{{grid-template-columns:1fr}}.ip-hero{{position:static;width:38mm;height:38mm;margin:8mm 0 0 auto}}}}@media print{{body{{background:white}}header,section,article,table{{break-inside:avoid}}}}
</style></head><body><main class="contract-{e(contract)}" data-fixed-template="固定模板06"><header><div class="header-copy"><div class="eyebrow">DP ACADEMIC SUPPORT</div><h1>{e(data.get("school"))}<br>{e(data.get("program"))} DP服务方案</h1><div class="tags"><span>{e(data.get("target_year"))}</span><span>目标：{e(data.get("target_score"))}</span><span>{e(data.get("product"))}</span><span>{e(data.get("scope"))}</span></div></div>{f'<img class="ip-hero" src="{ip}" alt="极致AI学业支持IP形象">' if ip else ''}</header>
{situation}<section><div class="section-title"><span>COURSE MATCH</span><h2>课程与服务匹配</h2></div>{course_map}</section>
{fixed_cards(fixed["core_value_title"], fixed["core_value"], "page-two" if contract == "D01" else "")}
<section class="assurance"><div class="section-title"><span>EXECUTION SYSTEM</span><h2>{e(fixed["assurance_process_title"])}</h2></div><div class="grid">{"".join(f"<article><p>{e(x)}</p></article>" for x in fixed["assurance_process"])}</div></section>
...
def main():
    ...
    ip = asset_uri(skill / "assets/ip/study-dashboard.jpg") if data.get("contract", "D01") == "D01" else ""
    output.write_text(client_html(data, fixed, ip), encoding="utf-8")
    Path(str(output) + ".internal.html").write_text(internal_html(data, fixed, warnings), encoding="utf-8")
    Path(str(output) + ".internal.json").write_text(json.dumps({"contract": data.get("contract", "D01"), "warnings": warnings, "sources": data.get("sources", []), "quote_status": data.get("quote_status"), "rule": "Internal-only. Never merge this file into a client proposal."}, ensure_ascii=False, indent=2), encoding="utf-8")
