...
</style></head><body><main><header><div class="header-copy"><div class="eyebrow">DP ACADEMIC SUPPORT</div><h1>{e(data.get("school"))}<br>{e(data.get("program"))} DP服务方案</h1><div class="tags"><span>{e(data.get("target_year"))}</span><span>目标：{e(data.get("target_score"))}</span><span>{e(data.get("product"))}</span><span>{e(data.get("scope"))}</span></div></div>{f'<img class="ip-hero" src="{ip}" alt="极致AI学业支持IP形象">' if ip else ''}</header>
