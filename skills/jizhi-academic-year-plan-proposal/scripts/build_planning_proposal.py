...
def brand_header(data, label, logo):
    ...
    return f'''<header>{logo_html}<div class="eyebrow">ACADEMIC PLANNING</div><div class="contract-label">{e(label)}</div>
    <h1>{e(data.get("school"))}<br><em>{e(data.get("program"))}</em></h1><div class="tags">{"".join(f'<span>{e(x)}</span>' for x in tags if x)}</div></header>'''
...
def client_body(data, fixed, contracts, logo):
    contract = data.get("contract", "T01")
    out = brand_header(data, contracts[contract]["label"], logo)
    ...
    if contract == "T00":
        ...
        out += f'<section><div class="section-title"><span>COURSE MATCH</span><h2>课程与服务匹配</h2></div>{courses}</section>'
    elif contract == "T01":
...
def client_html(data, fixed, contracts, logo):
    body = client_body(data, fixed, contracts, logo)
...
@page{{size:A4;margin:0}}*{{box-sizing:border-box}}:root{{--navy:#082f6b;--blue:#0765f5;--cyan:#0bc8b5;--ice:#eef7ff;--line:#cce1f5;--ink:#17243c}}body{{margin:0;background:#dcecff;color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif;line-height:1.55;letter-spacing:0}}main{{width:210mm;margin:auto;background:#fff;min-height:297mm}}header{{position:relative;padding:16mm 15mm 13mm;background:linear-gradient(120deg,#f5faff 0%,#eef8ff 62%,#dffcf7 100%);border-bottom:1px solid var(--line)}}.logo{{width:47mm;height:auto;display:block;margin-bottom:12mm;object-fit:contain}}.eyebrow,.section-title span{{font-size:10px;font-weight:800;letter-spacing:1.2px;color:#078b7d}}.contract-label{{position:absolute;right:15mm;top:16mm;padding:5px 12px;border-radius:5px;color:white;background:linear-gradient(90deg,var(--blue),var(--cyan));font-size:11px;font-weight:700}}h1{{margin:5px 0 9px;color:var(--navy);font-size:31px;line-height:1.2}}h1 em{{font-style:normal;color:var(--blue)}}.tags{{display:flex;gap:7px;flex-wrap:wrap}}.tags span{{padding:4px 9px;border:1px solid #bfe0ff;border-radius:5px;background:white;color:#315477;font-size:11px}}section{{padding:8mm 15mm;border-bottom:1px solid #e5eef8}}.section-title{{display:flex;align-items:baseline;gap:10px;margin-bottom:12px}}h2{{margin:0;color:var(--navy);font-size:20px;line-height:1.25}}h3{{margin:0 0 5px;color:var(--navy);font-size:14px}}p,li{{font-size:12px}}ul{{margin:6px 0;padding-left:18px}}.overview{{display:grid;grid-template-columns:1.45fr 1fr;gap:18px}}.metrics{{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;align-self:center}}.metrics b{{padding:12px 6px;text-align:center;color:var(--blue);font-size:24px;border:1px solid var(--line);border-radius:6px}}.metrics small{{display:block;color:#60748c;font-size:9px}}.grid{{display:grid;grid-template-columns:repeat(2,1fr);gap:9px}}.grid.six{{grid-template-columns:repeat(3,1fr)}}article{{padding:11px;border:1px solid var(--line);border-radius:6px;background:#fbfdff}}article p{{margin:0;color:#5c6f87}}table{{width:100%;border-collapse:separate;border-spacing:0;font-size:10px;border:1px solid var(--line);border-radius:6px;overflow:hidden}}th,td{{padding:7px 8px;text-align:left;vertical-align:top;border-right:1px solid var(--line);border-bottom:1px solid var(--line)}}th:last-child,td:last-child{{border-right:0}}tr:last-child td{{border-bottom:0}}th{{color:var(--navy);background:#eaf5ff;font-weight:700}}.course-table th:nth-child(1){{width:12%}}.course-table th:nth-child(2){{width:17%}}.course-table th:nth-child(3){{width:25%}}.course-table th:nth-child(4){{width:18%}}.service-band{{color:white;background:linear-gradient(110deg,#075be6,#0abdaf)}}.service-band h2,.service-band h3,.service-band .section-title span{{color:white}}.service-band .lead{{font-size:14px;font-weight:700}}.rhythm{{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}}.service-band article{{background:rgba(255,255,255,.12);border-color:rgba(255,255,255,.26)}}.service-band article p{{color:white}}.team ul{{display:grid;grid-template-columns:repeat(2,1fr);gap:3px 18px}}.split{{display:grid;grid-template-columns:1fr 1fr;gap:12px}}.split article:first-child{{border-top:5px solid var(--blue)}}.split article:last-child{{border-top:5px solid var(--cyan)}}.quote-total{{display:flex;justify-content:flex-end;align-items:baseline;gap:10px;margin-top:14px;color:#50627a}}.quote-total strong{{font-size:26px;color:var(--blue)}}@media(max-width:760px){{main{{width:100%}}.overview,.grid.six,.grid,.split,.rhythm{{grid-template-columns:1fr}}.metrics{{grid-template-columns:repeat(3,1fr)}}}}@media print{{body{{background:white}}main{{width:100%}}header,section,article,table{{break-inside:avoid}}}}
</style></head><body><main>{body}</main></body></html>'''.replace(
        '@media print{body{background:white}main{width:100%}header,section,article,table{break-inside:avoid}}',
        '@media print{body{background:white}main{width:100%}.overview{grid-template-columns:1.45fr 1fr}.grid{grid-template-columns:repeat(2,1fr)}.grid.six{grid-template-columns:repeat(3,1fr)}.split{grid-template-columns:1fr 1fr}.rhythm{grid-template-columns:repeat(3,1fr)}.page-two{break-before:page}article,table{break-inside:avoid}section{break-inside:auto}}'
    )
...
def main():
    ...
    output.write_text(client_html(data, fixed, contracts, logo), encoding="utf-8")
